...
CHANNEL_USERNAME = "@VISWAxABHI"
DEV_USERNAME = "@Daiyan_FF"
DB_PATH = "bot_database.db"
TOKEN_JSON_PATH = "access_token.json"
...
# ===================== TOKEN PROCESSING =====================
def generate_hex_content(access_token):
    """Generate hex content from access token"""
    try:
        # Your actual token processing logic here
        # This is a placeholder - replace with your actual implementation
        import uuid
        open_id = str(uuid.uuid4())[:8]
        hex_content = f"{access_token}:{open_id}"
        return hex_content, open_id
...
async def handle_token_input(update, context):
    ...
    context.user_data["awaiting_token"] = False
    server_url = f"http://{LOCAL_IP}:{PORT}/{user_id}/"
...
def run_bot():
    try:
        app_bot = Application.builder().token(BOT_TOKEN).concurrent_updates(True).build()
...
@app.route('/health')
def health():
    return {"status": "healthy", "users": db_get_total_users()}

# ===================== INIT =====================
init_database()
MAINTENANCE_MODE = db_get_maintenance()

# Start bot only if not running on Vercel
if os.environ.get("VERCEL") != "1":
    threading.Thread(target=run_bot, daemon=True).start()
    print(f"🚀 All systems operational!")

# The 'app' variable is used by Vercel as the WSGI handler